...
var getUrl = function(segment, request, response){
	console.log("redirect url from mysql!!!!!!!!!!!!!!!!!!");
...
function getStats(segment, request, response){
	pool.getConnection(function(err, con){
		...
		con.query(cons.get_query.replace("{SEGMENT}", con.escape(segment)), function(err, rows){
			...
			if(err || rows.length == 0){
				console.log("From getStats() in logic.js; get_query returns 0 rows..");
				...
				con.query(cons.stats_query.replace("{URL_ID}", url_id), function(err, rows){
					if(err || rows.length == 0){
						response.send({result: false, errorCode: 2});
						console.log("From getStats() in logic.js; stats_query returns 0 rows..");
					}else{
						var resultSet = {};
						...
						for (var i = 0; i < rows.length; ++i){
							var newDate = new Date(rows[i].clickdate).toDateString() + ", " +
								new Date(rows[i].clickdate).toLocaleTimeString();;
							rows[i].clickdate = newDate;
						}
						resultSet.rows = rows;
						resultSet.counts = rows.length;
						response.send(resultSet);
						console.log("From getStats() in logic.js; " + rows.length + " rows returned..");
					}
				}); //end of query()
			}
		});
